...
# Slight modification to the PLS methods from the scikit-learn package originally
# by Edouard Duchesnay <edouard.duchesnay@cea.fr>. 
# For details see: https://scikit-learn.org/stable/modules/cross_decomposition.html#cross-decomposition .
# The only modification made is to choose the rank (or number of components of the model) in the 
# predict method separately from the n_components in the __init__ method to enable predictions
# with alternative levels of R without re-fitting the model each time.
...
class _PLS(TransformerMixin, RegressorMixin, MultiOutputMixin, BaseEstimator,
    ...
    def predict(self, X, rank, copy=True):
        ...
        self.x_rotations_ = np.dot(
            self.x_weights_[:,:rank],
            pinv2(np.dot(self.x_loadings_[:,:rank].T, self.x_weights_[:,:rank]),
                  check_finite=False))

        self.coef_ = np.dot(self.x_rotations_, self.y_loadings_[:,:rank].T)
...
class CCA(_PLS):
    ...
    def __init__(self, n_components=2, *, scale=True,
        ...
        super().__init__(n_components=n_components, scale=scale,
                         ...
                         algorithm="svd", max_iter=max_iter, tol=tol,
                         copy=copy)
